...
- **`"flycam"`-driven cameras (legacy tag or explicit `camera_mode: Flycam`) do not render GLB `Prop`/`Actor` models** — confirmed bug, only `kind: Primitive` geometry renders under a flycam view. No bundled `ironhold-lib` example project combines flycam with a real GLB Prop, so this gap has gone untested upstream. Workaround: use a `"player"`-tagged prefab with movement zeroed and `camera_mode: Fixed((position: ..., look_at: ..., fov: ...))` instead — confirmed to render GLB content correctly (see `showcase_camera`/`menu_camera` in `prefabs/prefabs.ron`). This costs free-fly exploration (Fixed cameras don't move); see `planning/claude_suggestions.md` for the full repro.
