...
To load a specific scene directly (bypassing the project's `initial_scene`), append `&scene=<path>` relative to the project root, e.g. `http://localhost:8080/?project=scifi_fps&scene=scenes/main.scene.ron`. This is a real engine feature (`ironhold_web/src/lib.rs`, also used by ironhold-lib's own `test_web.py` harness) — no code or RON change needed to test a non-default scene.
...
`update_lib.py` also downloads `docs/` (offline reference) and the engine-provided `docs/engine_assets_claude.md` / `docs/engine_projects_claude.md` reference files at the same pinned SHA. These filenames are upstream names, not Claude-specific workflow requirements; they are gitignored and can be regenerated with `update_lib.py` at any time.

`update_lib.py` prints which `docs/*.md` files changed content since the last local download (diffed against the previous local copy, not a network fetch of the old SHA). **After any update where docs changed, review `AGENTS.md` and the relevant `.Codex/agents/*.md` files for drift** — RON schema/capability changes in this engine have previously made agent examples silently stale (wrong field nesting, fields that no longer exist, capability notes that were true before the update but aren't anymore). This review is manual/semantic, not automated — the printed file list only says *something* changed in that reference file, not what's now wrong in our own docs.
...
A prefab with `components.tags: ["player"]` spawns a character controller. By default (no `camera_mode` set) it gets a third-person orbit camera. As of `camera_modes` v2 (engine commit `545cfa8`+), a `components.camera_mode: FirstPerson((eye_offset: ..., sensitivity: ..., min_pitch: ..., max_pitch: ..., fov: ...))` block gives a true locked first-person camera — camera at head height, mouse-look pitch, character yaw turns with the view. Other `camera_mode` variants: `Orbit`, `Follow` (fixed-offset, no free orbit — top-down/side-scroller), `Fixed` (static world position, `look_at`/`look_at_entity`), `Flycam` (free-fly, same behavior as the legacy `"flycam"` tag), `Party` (internal, built from `split:`/`party:`, not authored directly). Named-field variants need **double parens** (`FirstPerson((...))`, not `FirstPerson(...)`). Scenes can also declare a `camera_modes:` registry and switch at runtime via `Action::SetCameraMode(mode: "key", owner_player: Option<u32>)`, with an optional `transition: (duration_secs, ease)` for eased blends; `"default"` is a reserved key that restores the scene-authored starting mode. Full schema: `docs/20_data_formats.md` (search `CameraModeDef`).

For a free-flying showcase/debug camera (not a locked player view), use a `"flycam"` tagged prefab — driven by mouse + WASD.
...
- **`CameraConfig`/`CameraModeDef` pitch fields (`min_pitch`/`max_pitch`) are in radians**, unlike transform `rotation_euler_deg` — no `_deg` suffix means radians throughout this part of the schema (confirmed for `Orbit` and reused as-is by `FirstPerson`, which has no documented per-field defaults of its own, only a variant-summary listing).
...
- **To test a scene other than the project's `initial_scene`, use `&scene=<path>` in the URL** (see Local development above) instead of temporarily editing `initial_scene` in `.project.ron` and reverting it afterward.
...
- **A `"flycam"`-tagged entity's `rotation_euler_deg` yaw of `(0,0,0)` faces `-Z`, not `+Z`.** This is standard Bevy camera convention (identity rotation looks down local -Z), but easy to get backwards when a scene's other content was laid out assuming "yaw 0 faces forward down +Z" (e.g. copying a corridor/player-spawn layout convention). Symptom looks exactly like "GLB Props never render under flycam" — the ground/primitives still show (matching the camera's actual empty facing direction), while every GLB Prop in the scene appears invisible because the camera is quietly facing directly away from them. It is **not** a rendering bug and pitch/distance/waiting-longer changes nothing — the fix is either flipping yaw by 180° or re-deriving the camera's position so content sits on its actual `-Z` side (see `ironhold-lib`'s own `custom_materials` demo, which places its flycam on the `+Z` side of its content for exactly this reason). A top-down test shot (`rotation_euler_deg: (-90,0,0)`) is a good sanity check when a flycam view looks unexpectedly empty — pitch -90 renders correctly regardless of yaw, so if content suddenly appears from directly above but not from any level angle, suspect this yaw issue before suspecting the renderer.
